...
$(function () {
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            {label: '编号', name: 'uid', width: 75, hidden: true},
            {label: '用户名', name: 'username', width: 200},
            {label: '名称', name: 'name', width: 200},
            {label: '角色ID', name: 'roleid', width: 200},
            {
                label: '角色', name: 'role', width: 200, formatter: function (cellvalue, options, cell) {
                        var role = cell.roleid;
                        for(var i = 0 ; i < roleArray.length ; i++){
                            if(roleArray[i].id == role){
                                return roleArray[i]['description'];
                            }
                        }
    ...
    $("#modifyLearnBtn").click(function () {
        ...
        if (rows == 0) {
            ...
            $("#add_role").val(data.roleid).prop('disabled',true);
...
function saveLearn() {
    ...
    var role = $('#add_role').find("option:selected").val();
    $.ajax({
        url: task,
        cache: false,
        dataType: 'json',
